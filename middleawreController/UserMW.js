...
const registerMiddleware = [
    body('email').notEmpty().isEmail().withMessage('email field must be valid email'),
    body('name').notEmpty().isString().isLength({min:3,max:10}).withMessage('name field must be minimum 3 and maximum 10'),
    body('password').notEmpty().isStrongPassword().withMessage('should be a strong password'),
    notAuthenticated,
    notRegistered
];
