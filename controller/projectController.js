...
const addProject = async (req, res) => {
    ...
    const isPublic =
        req.body.isPublic === true || req.body.isPublic === undefined;
    const projectPath = `./uploads/${
    ...
    const configuration =
        req.body.configuration < 24 || !req.body.configuration
            ? 28
...
const getProject = async (req, res) => {
    ...
    const project = await Project.findOne({projectName}, {currentPath: 1});
...
const editProject = async (req, res) => {
    ...
    if (!result)
        return res
            .status(404)
            .send({message: "Failed uploading the file try again"});

    return res
        .status(200)
        .send({message: "Edit project request has been sent"});
};
...
const getPending = async (req, res) => {
    ...
    return res.status(200).send({
        message: "Pendings retrieved successfully",
        pendings: project.pendings,
    });
};
...
const acceptPending = async (req, res) => {
    ...
    let publicPath = isPublic || isPublic === undefined ? "public" : "private";
    // Getting the path for pendings and uploads.
    const pendings = `./pending/${publicPath}/${fileName}`;
    const uploads = `./uploads/${publicPath}/${projectName}/${fileName}`;

    // moving the file from pendings to uploads
    if (!moveFile(pendings, uploads, fileName))
...
const rejectPending = async (req, res) => {
    const {projectName, isPublic} = req.params;
    ...
    deleteFolder(pendings);
    await Project.findOneAndUpdate(
        {projectName},
        {$pull: {pendings: {fileName}}}
    );

    return res.status(200).send({message: "Project has been rejected."});
};
