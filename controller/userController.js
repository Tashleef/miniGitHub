...
const loginController = async (req, res) => {
    ...
    try {
        ...
        const token = await Token.findOne({userId: user._id}).select(
            "accessToken refreshToken -_id"
        );
        let refreshToken = token.refreshToken;
        let accessToken = token.accessToken;
        if (!verifytoken(refreshToken, process.env.REFRESH_TOKEN))
            refreshToken = generateRefreshToken(userObject);
        if (!verifytoken(accessToken, process.env.ACCESS_TOKEN))
